...
void loop_over_exponents(
  ...
  if (d == D-1) {
    ...
    ASSERT_EQ(Portage::moment_to_index<D>(order, exponents), index);
...
void run_moment_index_test() {
  ...
  std::tie(order,exponents) = Portage::index_to_moment<D>(0);
  ASSERT_EQ(order, 0);
  for (int d = 0; d < D; ++d) {
  ...
  for (int d1 = 0; d1 < D; ++d1) {
    std::tie(order,exponents) = Portage::index_to_moment<D>(d1+1);
    ASSERT_EQ(order, 1);
