...
TEST(Simple_State_Wrapper, manageMeshField2) {
  ...
  std::shared_ptr<StateVectorUni<>> out =
      std::dynamic_pointer_cast<StateVectorUni<>>(pv2);
...
TEST(Simple_State_Wrapper, manageMeshFieldTemplate) {
  ...
  // create a uni state vector
  std::shared_ptr<StateVectorUni<>> pv =
      std::make_shared<StateVectorUni<>> ("field", Entity_kind::CELL,
                                          data);
  ...
  // create a uni state vector
  std::shared_ptr<StateVectorUni<>> pv2 =
      std::make_shared<StateVectorUni<>> ("field2", Entity_kind::CELL,
                                          data2);
...
TEST(Simple_State_Wrapper, mixedFields) {
  ...
  // create a uni state vector
  auto spsd = std::make_shared<StateVectorUni<>> ("temperature",
                                                  Entity_kind::CELL,
                                                  sdata);
...
TEST(Simple_State_Wrapper, mixedFields2) {
  ...
  // create a uni state vector
  auto sp2 = std::make_shared<StateVectorMulti<Point<2>>> ("mm centroid",
                                                           data2);
  ...
  for (auto& kv : out2->get_data()) {
    ...
    for (int j = 0; j < out.size(); j++) {
      ...
      ASSERT_EQ(data2[kv.first][j][0], out2->get_data()[kv.first][j][0]);
       // y coord
      ASSERT_EQ(data2[kv.first][j][1], out2->get_data()[kv.first][j][1]);
    }
  }
  ...
  for (int i = 0; i < data4.size(); i++) {
    ...
    ASSERT_EQ(data4[i][0], out4->get_data()[i][0]);
     // y coord
    ASSERT_EQ(data4[i][1], out4->get_data()[i][1]);
  }
...
TEST(Simple_State_Wrapper, test4Cell) {
  ...
  ASSERT_THROW(
      manager.add(std::make_shared<StateVectorMulti<>>("density_bad_shape1",
                                                       density_bad_shape1)),
      std::runtime_error);
  ...
  ASSERT_THROW(
      manager.add(std::make_shared<StateVectorMulti<>>("density_bad_shape2",
                                                       density_bad_shape2)),
      std::runtime_error);
  ...
  std::unordered_map<int, std::vector<double>> density_bad_shape3{
    ...
    {7, {100., 100.1}}};
  ASSERT_THROW(
      manager.add(std::make_shared<StateVectorMulti<>>("density_bad_shape3",
                                                       density_bad_shape3)),
      std::runtime_error);
  ...
  ASSERT_EQ(out->get_type(), Wonton::Field_type::MULTIMATERIAL_FIELD);
  ...
  std::unordered_map<int, std::vector<Point<2>>> centroid {
    {1, {Point<2>{.5, .5}, Point<2>{1.5, .5}, Point<2>{.5, 1.5}, Point<2>{1.5,
                                                                       1.5}}},
    {3, {Point<2>{.55, .55}, Point<2>{1.55, .55}}},
    {5, {Point<2>{.45, .45}, Point<2>{.45, 1.45}}}};

  // add the scalar field
  manager.add(std::make_shared<StateVectorMulti<Point<2>>> ("centroid",
                                                            centroid));
