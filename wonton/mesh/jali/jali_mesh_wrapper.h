...
/*!
  ...
  Jali_Mesh_Wrapper implements methods required for Wonton mesh
...
class Jali_Mesh_Wrapper : public AuxMeshTopology<Jali_Mesh_Wrapper> {
 public:
  ...
  Wonton::Element_type cell_get_element_type(int const cellid) const {
    ...
    return jali2wonton_elemtype(ctype);
  }
 ...
 private:
  ...
  Wonton::Entity_type jali2wonton_type(Jali::Entity_type etype) const {
    static Wonton::Entity_type jali2wonton_type_[5] =
        {DELETED, PARALLEL_OWNED, PARALLEL_GHOST, BOUNDARY_GHOST, ALL};
    return jali2wonton_type_[static_cast<int>(etype)];
  }
  
  Wonton::Element_type jali2wonton_elemtype(Jali::Cell_type elemtype) const {
    static Wonton::Element_type jali2wonton_elemtype_[9] =
        {UNKNOWN_TOPOLOGY, TRI, QUAD, POLYGON, TET, PRISM, PYRAMID, HEX,
         POLYHEDRON};
    return jali2wonton_elemtype_[static_cast<int>(elemtype)];
  }
