 ...
 */
bool vdd_eq(const std::vector<Wonton::Point2> &a,
            const std::vector<Wonton::Point2> &b, const double eps=1e-12)
{
 ...
 */
void coordinates_canonical_rotation(const Wonton::Point2 center_node,
                                    std::vector<Wonton::Point2> * const xylist)
{
    int i = 0;
    auto angle = [&]() {
...
void dual_cell_coordinates_canonical_rotation(
    const Wonton::Flecsi_Mesh_Wrapper &mesh_wrapper,
    int const nodeid,
    std::vector<Wonton::Point2> * const xylist) {
  Wonton::Point2 center_node;
...
TEST(Flecsi_Mesh, ccw) {
  mesh_t mesh;
  Wonton::make_mesh_cart2d(0.0, 1.0, 0.0, 1.0, 2, 2, mesh);
  Wonton::Flecsi_Mesh_Wrapper mesh_wrapper(mesh);
...
//   mesh_wrapper.dual_cell_get_node_adj_cells(center_node, Wonton::ALL, &adjdualcellids); 
