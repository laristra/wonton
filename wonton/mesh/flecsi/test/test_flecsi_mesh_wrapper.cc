...
bool vdd_eq(const std::vector<Wonton::Point<2>> &a,
    const std::vector<Wonton::Point<2>> &b, const double eps = 1e-12)
{
...
void coordinates_canonical_rotation(
      const Wonton::Point<2>& center_node,
...
bool faceted_poly_ok(std::vector<Wonton::Point<3>> const & fctpoints,
  ...
  Wonton::Point<3> gcen(0.0, 0.0, 0.0);  // Geometric center
  ...
  Wonton::Point<3> cfctcen, ctetcen;
  for (int i = 0; i < nfacets; i++) {
    ...
    for (int j = 1; j < nfpoints-1; j++) {
      ...
      Wonton::Vector<3> outnormal = cross(v0, v1);
  ...
  Wonton::Point<3> dummy_point(-99, -99, -99);
  if (!Wonton::approxEq(expected_centroid, dummy_point) &&
      !Wonton::approxEq(expected_centroid, cfctcen, 1.0e-08)) {
  ...
  if (!Wonton::approxEq(expected_centroid, ctetcen, 1.0e-08)) {
    std::cerr << "Expected centroid " << expected_centroid[0] << " " <<
        expected_centroid[1] << " " << expected_centroid[2] << "\n";
...
TEST(FleCSI_Mesh, vdd_eq) {
    std::vector<Wonton::Point<2>> a, b, c;
    a = {{0.25, 0}, {0.25, 0.25}};
    b = {{0.25, 0}, {0.25, 0.25}};
    c = {{0.25, 0.25}, {0.25, 0}};
...
TEST(FleCSI_Mesh, coordinates_canonical_rotation) {
    std::vector<Wonton::Point<2>> xylist, xylist_canonical;
    xylist_canonical = {
                {0.75, 0.5},
                {0.75, 0.75},
                {0.5, 0.75},
...
TEST(FleCSI_Mesh_Wrapper, Get_Neighbor_Cells) {
  ...
  mesh_wrapper.dual_cell_get_node_adj_cells(center_node, Wonton::ALL,
                                            &adjdualcellids);
...
TEST(FleCSI_Mesh_Wrapper, Get_Exterior_Flag) {
  ...
  int nnodes = mesh_wrapper.num_entities(Wonton::Entity_kind::NODE,
                                         Wonton::Entity_type::ALL);
  for (int n = 0; n < nnodes; n++) {
    ...
    mesh_wrapper.node_get_corners(n, Wonton::Entity_type::ALL, &nodecorners);
  ...
  for (int c = 0; c < ncells; ++c) {
    ...
    for (auto const & s : csides) {
      ...
      if (s2 != -1) {
        ...
        std::array<Wonton::Point<2>, 3> scoords;
        ...
        std::array<Wonton::Point<2>, 3> scoords2;
        ...
        if (scoords[0][0] == scoords2[1][0] &&
          ...
          Wonton::Vector<2> vec0 = scoords[1]-scoords[0];
          Wonton::Vector<2> vec1 = scoords[2]-scoords[0];
...
TEST(FleCSI_Mesh_Wrapper, MESH_SIDES_3D) {
  ...
  for (int c = 0; c < ncells; ++c) {
    ...
    for (auto const & s : csides) {
      ...
      int n0 = mesh_wrapper.side_get_node(s, 0);
      ASSERT_GE(n0, 0);
      Wonton::Point<3> npnt0;
      mesh_wrapper.node_get_coordinates(n0, &npnt0);

      int n1 = mesh_wrapper.side_get_node(s, 1);
      ASSERT_GE(n1, 0);
      Wonton::Point<3> npnt1;
      mesh_wrapper.node_get_coordinates(n1, &npnt1);
      ...
      std::array<Wonton::Point<3>, 4> scoords;
      mesh_wrapper.side_get_coordinates(s, &scoords);
      ...
      // Now get the side coordinate in the positive volume order
      std::array<Wonton::Point<3>, 4> scoords2;
      mesh_wrapper.side_get_coordinates(s, &scoords2, true);
      ...
      if (scoords[0][0] == scoords2[1][0] &&
        ...
        Wonton::Vector<3> vec0 = scoords[1]-scoords[0];
        Wonton::Vector<3> vec1 = scoords[2]-scoords[0];
        Wonton::Vector<3> vec2 = scoords[3]-scoords[0];
        Wonton::Vector<3> cpvec = cross(vec0, vec1);
  ...
  for (int c = 0; c < ncells; ++c) {
    ...
    for (auto const & w : cwedges) {
      ...
      if (wcoords[1][0] == wcoords2[2][0] &&
        ...
        Wonton::Vector<2> vec0 = wcoords[1]-wcoords[0];
        Wonton::Vector<2> vec1 = wcoords[2]-wcoords[0];
        double cpvec = cross(vec0, vec1);
        double altvolume = cpvec;
        ASSERT_TRUE(altvolume*volume < 0.0);
...
TEST(FleCSI_Mesh_Wrapper, MESH_WEDGES_3D) {
  ...
  for (int c = 0; c < ncells; ++c) {
    ...
    for (auto const & w : cwedges) {
      ...
      // Now get the wedge coordinate in the positive volume order
      std::array<Wonton::Point<3>, 4> wcoords2;
      mesh_wrapper.wedge_get_coordinates(w, &wcoords2, true);
      ...
      if (wcoords[1][0] == wcoords2[2][0] &&
        ...
        Wonton::Vector<3> vec0 = wcoords[1]-wcoords[0];
        Wonton::Vector<3> vec1 = wcoords[2]-wcoords[0];
        Wonton::Vector<3> vec2 = wcoords[3]-wcoords[0];
        Wonton::Vector<3> cpvec = cross(vec0, vec1);
        double altvolume = dot(cpvec, vec2);
        ASSERT_TRUE(altvolume < 0.0);
...
TEST(FleCSI_Mesh_Wrapper, MESH_CORNERS_2D) {
  ...
  int nnodes = mesh_wrapper.num_entities(Wonton::NODE, Wonton::ALL);
  for (int n = 0; n < nnodes; ++n) {
    std::vector<int> corners;
    mesh_wrapper.node_get_corners(n, Wonton::ALL, &corners);
...
/*TEST(FleCSI_Mesh_Wrapper, Skewed_2DCell_Geometry) {
  ...
  Wonton::Point<2> cen_computed;
  mesh_wrapper.cell_centroid(0, &cen_computed);
  ASSERT_TRUE(Wonton::approxEq(cen, cen_computed, 1.0e-8));
}
*/
...
TEST(FleCSI_Mesh_Wrapper, Skewed_3DCell_Geometry) {
  ...
  // facets use a different face point than Wonton mesh wrapper. So
  // don't compare this
  double cfarea = -1.0;
