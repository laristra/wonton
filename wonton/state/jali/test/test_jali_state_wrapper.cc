...
TEST(Jali_State_Wrapper, DataTypes) {
  ...
  wrapper.mesh_add_data<double>(Wonton::CELL, "d1", dtest);
  double dval = 123.456;
  wrapper.mesh_add_data<double>(Wonton::CELL, "d2", dval);
...
TEST(Jali_State_Wrapper, MMState) {
  ...
  statewrapper.mesh_add_data(Wonton::Entity_kind::CELL, "cell_density", 0.0);
  ...
  //  double *rho;
  //  statewrapper2.mesh_get_data(Wonton::Entity_kind::CELL, "cell_density", &rho);
