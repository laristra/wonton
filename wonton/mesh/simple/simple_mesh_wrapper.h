...
#ifndef WONTON_SIMPLE_MESH_WRAPPER_H_
#define WONTON_SIMPLE_MESH_WRAPPER_H_
...
namespace Wonton {
  /*!
    ...
    The methods implemented are those required elsewhere in Wonton to
...
class Simple_Mesh_Wrapper : public AuxMeshTopology<Simple_Mesh_Wrapper> {
 public:
  ...
  explicit Simple_Mesh_Wrapper(Simple_Mesh const & mesh,
                               ...
                               CoordSys coord_sys = CoordSys::Cartesian) :
      AuxMeshTopology<Simple_Mesh_Wrapper>(
        request_sides, request_wedges, request_corners),
      mesh_(mesh) {
    if (coord_sys == CoordSys::CylindricalAxisymmetric)
  ...
  */
  ...
  /*!
    @brief Get the coordinates of a specific node as a Wonton::Point.
...
#endif  // WONTON_SIMPLE_MESH_WRAPPER_H_
